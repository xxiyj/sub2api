...
func (s *UpdateService) RollbackToVersion(ctx context.Context, version string) error {
	target := normalizeReleaseVersion(strings.TrimSpace(version))
	if target == "" {
...
func (s *UpdateService) fetchRollbackCandidates(ctx context.Context) ([]*GitHubRelease, error) {
	releases, err := s.githubClient.FetchRecentReleases(ctx, getUpdateGitHubRepo(), rollbackFetchPageSize)
	if err != nil {
		return nil, err
	}
	...
	for _, r := range releases {
		if !isCustomRelease(r) || r.Prerelease {
			continue
		}
...
func (s *UpdateService) fetchLatestCustomRelease(ctx context.Context) (*GitHubRelease, error) {
	...
	releases, err := s.githubClient.FetchRecentReleases(ctx, repo, 30)
	if err != nil {
		return nil, err
	}
